...
#SBATCH --output=out256.txt
#SBATCH --error=err256.txt
...
for index in "${!sizes[@]}"; do
  for ((k = 0; k < 20; k++)); do
    time srun --cpu_bind=cores --distribution=block:block ./gof_mpi_chessboard.o -s "${sizes[index]}","${sizes[index]}" -R "${repetitions[index]}" >> Tasks256_S"${sizes[index]}".txt 2>&1
  done
done
